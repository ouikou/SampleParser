/* Generated By:JJTree: Do not edit this line. ASTAbstractDeclarator.java Version 4.3 */
/* JavaCCOptions:MULTI=true,NODE_USES_PARSER=true,VISITOR=true,TRACK_TOKENS=false,NODE_PREFIX=AST,NODE_EXTENDS=,NODE_FACTORY=*,SUPPORT_CLASS_VISIBILITY_PUBLIC=true */
package parser.textmodel.ast;

public
class ASTAbstractDeclarator extends SimpleNode {
  public ASTAbstractDeclarator(int id) {
    super(id);
  }

  public ASTAbstractDeclarator(TMParser p, int id) {
    super(p, id);
  }

  public static Node jjtCreate(int id) {
    return new ASTAbstractDeclarator(id);
  }

  public static Node jjtCreate(TMParser p, int id) {
    return new ASTAbstractDeclarator(p, id);
  }

  /** Accept the visitor. **/
  public Object jjtAccept(TMParserVisitor visitor, Object data) {
    return visitor.visit(this, data);
  }
}
/* JavaCC - OriginalChecksum=2e728fc1cb71c094a9ea0e70e9dfb3dc (do not edit this line) */
